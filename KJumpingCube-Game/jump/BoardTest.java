...
public class BoardTest {
    ...
    public void testGet() {
        ...
        B.set(2, 5, 4, RED);
        B.set(5, 5, 2, BLUE);
        assertEquals("wrong number of spots", 4, B.get(2, 5).getSpots());
    ...
    @Test
    public void testEndJumpIfWinner() {
        Board B = new Board(2);
        B.toString();
        B.addSpot(BLUE, 1, 1);
        B.addSpot(BLUE, 2, 2);
        ...
        assertEquals(B.getWinner(), BLUE);
    }

    @Test
    public void testUpdateSquareCounts() {
        Board B = new Board(6);
        assertEquals(B.numRed(), 0);
        assertEquals(B.numBlue(), 0);

        B.addSpot(RED, 1, 1);
        assertEquals(B.numRed(), 1);
        assertEquals(B.numBlue(), 0);

        B.addSpot(BLUE, 6, 6);
        B.addSpot(BLUE, 5, 6);
        assertEquals(B.numRed(), 1);
        assertEquals(B.numBlue(), 2);

        B.addSpot(RED, 5, 6);
        assertEquals(B.numRed(), 2);
        assertEquals(B.numBlue(), 1);
    }
